...
export default function Register() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { confirmPassword: _unused, ...submitData } = form;
    ...
    return (
        <AuthLayout title="Create Account">
            {error && <p className="bg-red-500/20 text-red-100 border border-red-500/40 p-3 rounded-xl mb-4 text-sm text-center">{error}</p>}
            ...
            <form onSubmit={handleSubmit} className="space-y-4" autoComplete="off">
                ...
                <div className="relative">
                    ...
                    />
                    <button type="button" onClick={() => setShowConfirmPass(!showConfirmPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/50 hover:text-white">
                        {showConfirmPass ? <EyeOff size={20} /> : <Eye size={20} />}
                    </button>
                </div>
                
                <select className={inputStyle} value={form.role} onChange={e => setForm({...form, role: e.target.value})}>
                    <option value="user" className="bg-stone-900">User Role</option>
                    <option value="admin" className="bg-stone-900">Admin Role</option>
                </select>

                {form.role === 'admin' && (
                    <input 
                        className="w-full p-3 bg-red-900/20 border border-red-500/30 rounded-xl text-white placeholder-red-200 focus:outline-none focus:ring-2 focus:ring-red-400" 
